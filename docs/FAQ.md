...
What are the observable results caused by increasing or decreasing the value of K?

## Answer to Q8

Changing the value of K (the number of difference tokens) in Question-Guided Tokenizer (QDT) will create a direct and observable trade-off between model performance and computational efficiency.

### Observable Results of Decreasing K (K=4)

- __Efficiency:__ The model will be at its fastest and most computationally efficient. The downstream language model has very few tokens to process, reducing memory usage and speeding up inference.

- __Performance:__ The model's accuracy (e.g., its CIDEr score) will likely be lower. With only a few tokens, the model may not have enough visual evidence to understand complex scenes or answer questions that require looking at multiple locations. It might miss secondary, but still important, findings.

## Observable Results of increasing K (K=16 or 32)

- __Efficiency:__ The model will be slower and less computationally efficient. The language model now has to perform cross-attention over a larger set of tokens, increasing the computational load.

- __Performance:__ The model's accuracy will likely increase, as it now has a richer, more detailed representation of the visual differences. It can capture more complex changes and answer more nuanced questions. However, you will likely see diminishing returns. At some point, adding more tokens will not improve performance and may even hurt it by introducing irrelevant, noisy information.

### The Goal of the "K Sweep"

The "K sweep" experiment is designed to find the "sweet spot" on this trade-off curve, often called an efficiency frontier. The model's accuracy will be plotted against its computational cost for each value of K and find the optimal point where the most performance is gained for the least amount of computation.

___

## Q9

Why do we need masked residual modeling?

Before your model can answer questions about differences, its vision components—the Directional Residual Stack (DRS) and Question-Guided Tokenizer (QDT)—need a foundational understanding of what a radiological change even looks like.

## Answer to Q9

Masked Residual Modeling (MRM) is required to effectively pre-train the model's vision system to be "difference-sensitive" without relying on external text data like radiology reports. It's a self-contained "warm-up" that makes your final model more powerful and efficient.

### How to teach model to see change

Before the model can answer questions about differences, its vision components—the Directional Residual Stack (DRS) and Question-Guided Tokenizer (QDT)—need a foundational understanding of what a radiological change even looks like.

Other models, like PLURAL, solve this by pre-training on vast datasets of images paired with their text reports. While effective, this creates a major dependency on large, external text corpora.

### MRM's solution: Self-supervised learning

MRM is a more elegant and self-contained solution. It teaches the model to understand the context and structure of visual differences using only the images themselves.

Analogy: Think of it like learning a language. We could read an entire library of books (the PLURAL approach), or we could solve thousands of crossword puzzles (the MRM approach). The crossword puzzles force us to understand the relationships between words to fill in the blanks, teaching us the language's structure without needing to read full novels.

By masking parts of the difference maps and forcing the model to reconstruct them, you are teaching it the "grammar" of radiological change. This pre-training makes the model much more effective and data-efficient when we later fine-tune it on the actual question-answering task.

___

## Q10

How is our model designed to handle data imbalance?

## Answer to Q10

### 1. Self-Supervised Pre-training (MRM)

The Masked Residual Modeling (MRM) is your first line of defense against data imbalance.

- __How it helps:__ Before the model ever sees the imbalanced VQA labels, MRM forces it to learn the fundamental structure and patterns of radiological change from all images, regardless of whether the finding is common or rare. By learning to reconstruct masked patches of a rare finding like a "fracture," the model develops a robust visual understanding of it.

- __Benefit:__ This pre-training gives the model a strong, unbiased starting point. It prevents the model from developing an early bias towards just recognizing common findings like 'Pleural Effusion' simply because it sees them more often.

### 2. Counterfactual Regularizer (CFA-Reg)

The Counterfactual Regularizer is your second defense, which helps during the fine-tuning stage.

- __How it helps:__ The CFA-Reg forces the model to be robust and reason based on evidence, not just statistical frequency. For a rare finding, it doesn't just learn from the few positive examples. It also learns from the counterfactuals, such as seeing the finding and being asked if it "resolved."

- __Benefit:__ This teaches the model to look for the specific visual evidence of a finding rather than just guessing the most probable answer. It makes the model less likely to ignore a rare finding and more likely to correctly identify the absence of a common one, improving its overall reliability across the entire data distribution.
